...
# 성별 월급 평균표 만들기
sex_income <- welfare %>% 
  filter(!is.na(income)) %>%       ### 월급받는 사람 먼저 골라라
  group_by(sex) %>%      #### group_by : 어떤 그룹으로 나누어라
...
head(age_income)
qplot(welfare$aage_income)
ggplot(data = age_income, aes(x=age, y=mean_income)) + geom_line()   ### line 선
ggplot(data = age_income, aes(x=age, y=mean_income)) + geom_point()    ### point 점
ggplot(data = age_income, aes(x=age, y=mean_income)) + geom_col()    ### col 막대그래프
### 다. 년령대에 따른 월급 차이 ----- 어떤 연령대의 월급이 가장 많을까 ? -----------
# 분석절차
# 변수검토 및 전처리(연령대, 월급) 
...
sex_income <- welfare %>%   ## 전체 변수 조합 분석
  filter(!is.na(income)) %>% 
  group_by(ageg, sex) %>% 
  summarise(mean_income = mean(income))  
...
ggplot(data=sex_income, aes(x=ageg, y=mean_income, fill = sex)) + geom_col(position = "dodge") +
  scale_x_discrete(limits = c("young", "middle", "old"))        #### position = "dodge' 가로로 붙음.
...
### 마. 직업별 월급차이 --- 어떤직업이 월금을 가장 많이 받을까 ?
# 직업별 월급 문석하기
...
### 바. 성별 직업 빈도 - "성별로 어던 직업이 가장 많을까 ?   -------------------11.27 수업시자
# 분석절차
